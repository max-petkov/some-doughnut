...
function basicsGeometriesJS() {
    ...
    function render3d(config) {
        ...
        function gui() {
            ...
            gui.addColor(options, "color").name("Color:").onChange(function () {
                model.model.material.color.setHex(options.color);
                model.model2.material.color.setHex(options.color);
            });
            gui.add(options, "wireframe").name("Wireframe:").onChange(function (bol) {
                if (config.wireframe) {
                    model.line.visible = bol;
                    model.line2.visible = bol;
                };
            });
            gui.add(options, "opacityWireframe").min(0).max(1).step(0.01).name("Wireframe | Opacity:").onChange(function (num) {
                if (config.wireframe) {
                    console.log(num);
            ...
            gui.add(options, "isVisible").name("Remove color:").onChange(function (bol) {
